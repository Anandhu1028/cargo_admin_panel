            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="page-title-box d-md-flex justify-content-md-between align-items-center">
                        <h4 class="page-title">STEP 2 of 3 — Dubai / Origin Charges</h4>
                        <div>
            ...
            <div class="card shadow-sm border-0">
                <div class="card-body">
                    ...
                    <p class="small text-muted mb-4">
                        Mode of Transport: -- &nbsp;&nbsp; | &nbsp;&nbsp; Destination Port:
                        {{ strtoupper($calc->port) }}
                    </p>
                                                ...
                                                @foreach ($items as $row)
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            @if ($isPacking)
                                                                <input type="text"
                                                                    name="packing_unit[{{ $group }}][{{ $row['particular'] }}]"
                                                                    value="{{ $unit !== '-' ? $unit : '' }}"
                                                                    placeholder="Enter"
                                                                    class="form-control form-control-sm short-input text-center packing-unit-input"
                                                                    data-rate="{{ $rate }}" data-cbm="{{ $calc->cbm }}" data-roe="{{ $roe }}">
                                                            @elseif ($isOther)
                                                                <input type="text"
                                                            ...
                                                            @elseif ($isSpecial)
                                                                <input type="text"
                                                                    ...
                                                                    class="form-control form-control-sm short-input text-start special-desc">
                                                            @else
                                                                {{ $unit }}
                                                                <input type="hidden" name="unit[{{ $group }}][{{ $row['particular'] }}]" value="{{ $unit }}">
                                                            @endif
                                                        </td>
                                                        ...
                                                        <td class="text-end pe-3">
                                                            @if ($isLabour)
                                                                <input type="text" readonly class="form-control-plaintext text-end fw-semibold labour-amount"
                                                                    value="{{ number_format($amount, 2) }}">
                                                            @elseif ($isPacking)
                                                                <input type="text" readonly class="form-control-plaintext text-end fw-semibold packing-amount-display"
                                                                    value="{{ number_format($amount, 2) }}">
                                                            @elseif ($isOther)
                                                                <input type="text" readonly class="form-control-plaintext text-end fw-semibold other-amount-display"
                                                                    value="{{ number_format($amount, 2) }}">
                                                            @elseif ($isSpecial)
                                                                <input type="text" readonly class="form-control-plaintext text-end fw-semibold special-amount-display"
                                                                    value="{{ number_format($amount, 2) }}">
                                                            @else
                                                                <input type="text" readonly class="form-control-plaintext text-end fw-semibold static-amount"
                                                                    value="{{ number_format($amount, 2) }}">
                                                            @endif
                                                        </td>
                                                    </tr>
...
.short-input { max-width: 90px !important; display: inline-block; padding-right: 4px; }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll('.other-unit-input, .other-rate-input').forEach(inp => {
        inp.addEventListener('input', function() {
            const row = this.closest('tr');
            const cbm = toNumber(row.querySelector('.other-unit-input')?.dataset.cbm || 0);
            ...
            row.querySelector('.other-amount-display').value = formatNumber(cbm * rate * roe);
            updateAllGroupTotals();
        });
    });
