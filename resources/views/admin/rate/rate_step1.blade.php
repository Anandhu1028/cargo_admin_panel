      ...
      <div class="row">
        <div class="col-sm-12">
          <div class="page-title-box d-md-flex justify-content-md-between align-items-center">
            <h4 class="page-title">STEP 1 - 3</h4>
            <div>
        ...
        <div class="card-body">
          <form method="POST" action="{{ route('rate.step1.store') }}">
            ...
            <div class="row mb-3 align-items-end">
              <h5 class="fw-bold mb-3">Basic Shipment Details</h5>
              <div class="col-md-4">
                <label class="form-label fw-semibold">Customer Name</label>
                <select name="customer_id" id="customerSelect" class="form-select" required>
                  <option value="">Select Customer</option>
                  @foreach ($customers as $customer)
                                <option value="{{ $customer->id }}" {{ (old('customer_id', $calc->customer_id ?? '') == $customer->id) ?
                    'selected' : '' }}>
                                  {{ $customer->name }}
                                </option>
                  @endforeach
              ...
              <div class="col-md-1 d-flex align-items-end">
                <button type="button" class="btn p-0 border-0 bg-transparent" data-bs-toggle="modal"
                  data-bs-target="#addCustomer" title="Add Customer"
                  ...
                  onmousedown="this.style.transform='scale(0.9)';" onmouseup="this.style.transform='scale(1.2)';">
            ...
            <div class="row g-3">
              <div class="col-md-3">
                ...
                <input type="text" name="from_location" class="form-control"
                  value="{{ old('from_location', $calc->from_location ?? '') }}" placeholder="Enter city or district"
                  required>
              </div>
              ...
              <div class="col-md-3">
                ...
                <input type="text" name="to_location" class="form-control"
                  value="{{ old('to_location', $calc->to_location ?? '') }}" placeholder="Enter city or district"
                  required>
              </div>

              <div class="col-md-3">
                <label class="form-label">Port Name</label>
                <input type="text" name="port" class="form-control" value="{{ old('port', $calc->port ?? '') }}">
              </div>
              ...
              <div class="col-md-3">
                ...
                <select name="condition" class="form-select">
                  <option value="new" {{ old('condition', $calc->condition ?? '') == 'new' ? 'selected' : '' }}>New
                  </option>
                  <option value="used" {{ old('condition', $calc->condition ?? '') == 'used' ? 'selected' : '' }}>Used
                  </option>
                </select>
              </div>
            </div>
            ...
            <div class="row mt-4">
              <div class="col-md-3">
                <div class="form-check">
                  ...
                  <input type="checkbox" name="additional_packing" id="additional_packing" class="form-check-input"
                    value="1" {{ old('additional_packing', $calc->additional_packing ?? false) ? 'checked' : '' }}>
